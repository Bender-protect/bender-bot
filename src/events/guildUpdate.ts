...
export default new Event('guildUpdate', async(o, a) => {
    const { executor, reason } = (await a.fetchAuditLogs({ type: AuditLogEvent.GuildUpdate })).entries.first();
    if (!Bender.whitelistManager.isWhitelisted(a, executor.id, reason) && Bender.configsManager.state(a.id, 'guildUpdate_enable')) {
